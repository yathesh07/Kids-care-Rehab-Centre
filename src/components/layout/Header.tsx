                ...
                {item.children ? (
                  <>
                    <button
                      ...
                      className={cn(
                        'group flex items-center gap-1 rounded-full px-3 py-2 font-display text-[0.9rem] font-medium transition-colors',
                    ...
                    >
                      <span className="relative">
                        ...
                        <span
                          className={cn(
                            'absolute -bottom-1 left-0 h-[2px] w-full origin-left scale-x-0 rounded-full bg-brand-500 transition-transform duration-200 group-hover:scale-x-100',
                            openMenu === item.label && 'scale-x-100',
                          )}
                        />
                      </span>
                      <IconChevron
                    ...
                    <AnimatePresence>
                      {openMenu === item.label && (
                        <motion.div
                          ...
                          className="absolute left-0 top-full z-50 w-64 origin-top pt-2"
                          onMouseEnter={() => openWithCancel(item.label)}
                          onMouseLeave={scheduleClose}
                        >
                          <ul className="max-h-[70vh] overflow-y-auto rounded-card border border-ink-200 bg-white py-2 shadow-lift">
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        'group relative block rounded-full px-3 py-2 font-display text-[0.9rem] font-medium transition-colors',
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        ...
                        <span
                          className={cn(
                            'absolute -bottom-1 left-3 right-3 h-[2px] origin-left scale-x-0 rounded-full bg-brand-500 transition-transform duration-200 group-hover:scale-x-100',
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-0 z-50 flex h-dvh w-screen flex-col bg-white lg:hidden"
          >
            <motion.div
              initial={{ y: -16, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ duration: 0.22, delay: 0.03 }}
              className="container-page flex items-center justify-between border-b border-ink-200 py-3"
            >
              <Logo compact />
              <button
                type="button"
